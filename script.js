...
const posterItems = [
  ...
  { image: "BANNER/12.jpg", title: "海报 12" }
];
let activePoster = 0;
let posterTimer;
...
let eventNames = [];
let teamAwardItems = loadAwardCache();
...
function loadAwardCache() {
  try {
    const saved = JSON.parse(localStorage.getItem("sports-sheet-awards"));
    return saved || { red: [], yellow: [], blue: [] };
  } catch {
...
async function refreshScores() {
  ...
  try {
    ...
    const sheetRows = data.table.rows.map(row =>
      Array.from({ length: data.table.cols.length }, (_, index) => {
    ...
    localStorage.setItem("sports-sheet-awards", JSON.stringify(teamAwardItems));
...
function extractTeamAwards(rows) {
  ...
  rows.forEach(row => {
    ...
    const eventName = rowLabel;
    // Only numbered competition rows are real events. This excludes headings,
    // gender/medal labels, subtotals and the grand-total row.
    if (!Number.isFinite(eventNumber) || eventNumber <= 0 || !eventName) return;
      ...
      Object.entries(medals).forEach(([medal, columns]) => {
        columns.forEach((column, genderIndex) => {
          ...
          const amount = Number.isFinite(numericValue)
            ? numericValue
            : (String(rawValue ?? "").trim() && !/^[-—–0]$/.test(String(rawValue).trim()) ? 1 : 0);
...
const eventScheduleItems = [
  { event: "汤匙运送乒乓球 / Membawa Ping Pong dengan Sudu", categories: ["Kategori Prasekolah"] },
  { event: "瓶子装水赛 / Isi Air Dalam Botol", categories: ["Kategori Prasekolah", "Kategori Tahun 1", "Kategori Tahun 2"] },
  { event: "30米赛跑 / Larian 30M", categories: ["Kategori Prasekolah"] },
  { event: "100米赛跑 / Larian 100M", categories: ["Kategori Tahun 1", "Kategori Tahun 2", "Kategori 9 TKB", "Kategori 10 TKB", "Kategori 11 TKB", "Kategori 12 TKB"] },
  { event: "4 × 100米接力赛 / Relay 4 × 100M", categories: ["Kategori Terbuka"] }
];
...
function openInfoPanel(type) {
  ...
  document.querySelector("#infoContent").innerHTML = isProgramme
    ...
    : type === "events"
      ? `<ol class="info-list event-schedule-list">${eventScheduleItems.map((item, index) => `<li><strong>${String(index + 1).padStart(2, "0")}</strong><span><b>${item.event}</b><span class="category-lines">${item.categories.map(category => `<small>${category}</small>`).join("")}</span></span></li>`).join("")}</ol>`
      : type === "staff"
